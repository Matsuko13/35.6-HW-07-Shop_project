...
commentsRouter.post('/', async (req: Request<{}, {}, CommentCreatePayload>, res: Response) => {
  ...
  try {
    ...
    const id = uuidv4();
    await connection.query<ResultSetHeader>(COMMENT_INSERT_QUERY, [id, email, name, body, productId]);
    res.status(201);
...
commentsRouter.patch('/', async (req: Request<{}, {}, Partial<IComment>>, res: Response) => {
  try {
    ...
    const id = uuidv4();
    await connection.query<ResultSetHeader>(COMMENT_INSERT_QUERY, [
      id,
      newComment.email,
      newComment.name,
      newComment.body,
...
commentsRouter.delete('/:id', async (req: Request<{ id: string }>, res: Response) => {
  try {
    const [info] = await connection.query<ResultSetHeader>('DELETE FROM comments WHERE comment_id = ?', [
      req.params.id,
    ]);
